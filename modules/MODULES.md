...
| `modules/common.mk` | Shared build rules for every module — invoked via `make -C modules/<name> -f common.mk MODULE_NAME=<name>` from `modules/Makefile`. Reads repo / ref / `target_module` from the manifest via `manifest.mk`'s helpers. |
