...
> **Configuration files**: the bundled modules' `loadmodule` lines and
> per-module settings live in the auto-generated `redis-full.conf`, produced by
> `make build` (and regenerated on its own by `make sync-redis-conf`) — so after
> building, run `./src/redis-server redis-full.conf`. This is the same in a git
> checkout and in an extracted release tarball: `redis.conf` is the tracked,
> hand-edited Redis-core config in both, and carries no module lines. See
...
# `make` generated redis-full.conf for the modules it built — use it to
# start Redis with the bundled modules loaded:
./src/redis-server redis-full.conf
# Redis core only, no modules:
